...
<body>
  <div id="app">
    <nav class="navbar navbar-expand navbar-dark fixed-top" id="panel-navbar">
      <div class="container">
        <a class="navbar-brand d-sm-flex d-xs-none d-none me-3 align-items-center" href="{{ url('/') }}">
          <img src="{{ asset('images/logo-white.svg') }}" alt="{{ config('app.name', 'Laravel') }}" height="40px" />
          <div class="ms-3">{{ config('app.name', 'Laravel') }}</div>
        </a>
          ...
          <ul class="navbar-nav ms-auto align-items-center">
            ...
            <li class="nav-item dropdown dropdown-user">
              <a id="user-dropdown" class="nav-link dropdown-toggle" href="javascript:void()" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                ...
                <img src="{{ Auth::user()->profile_img }}" alt="{{ Auth::user()->fullname }}" />
                @else
                <img src="{{ asset('images/default.png') }}" alt="{{ Auth::user()->fullname }}" />
                @endif

                <div class="userinfo">
                  <div class="user-name">{{ Auth::user()->fullname }}</div>
                  <div class="badge badge-default">{{ Str::ucfirst(Auth::user()->getRoleNames()[0]) }}</div>
                </div>
              </a>

              <div class="dropdown-menu dropdown-menu-end" aria-labelledby="user-dropdown">
                <a class="dropdown-item" href="{{ route('logout') }}">{{ __('My Profile') }}</a>
    ...
    <aside class="sidebar-navbar">@include('layouts.panel.sidebar')</aside>
